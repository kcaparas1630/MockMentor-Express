...
import { QuestionService, UserService, InterviewService, AIService } from '../db';
...
export const submitUserResponse = async (req: AuthRequest, res: Response) => {
    try {
        ...
        if (!sessionId || !questionId || !answerResponse) {
            res.status(400).json({ error: 'Missing required fields: sessionId, questionId, answerResponse' });
            return;
        }
        ...
        const questionFeedback = await AIService.processAnswer(questionData);

        // Save the answer with AI feedback to session
        await InterviewService.saveAnswer(sessionId, questionId, answerResponse, question.question, questionFeedback);
        ...
        if (nextIndex >= allQuestions.length) {
            ...
            const completedInterview = await InterviewService.completeInterview(sessionId);
            ...
            const comprehensiveFeedback = await AIService.processAllAnswers(allAnswersData);

            // Update interview with final comprehensive feedback
            await InterviewService.updateInterviewFeedback(sessionId, comprehensiveFeedback);
...
export const getInterviewResults = async (req: AuthRequest, res: Response) => {
    try {
        ...
        const interview = await InterviewService.getInterviewWithResults(sessionId);
